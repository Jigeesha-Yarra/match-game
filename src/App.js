...
class App extends Component{
  ...
  clickTabItem = tabValue => {
    this.setState({category: tabValue})
  }

  imageClick = thumbnailUrl => {
    const {imgUrl} = this.state;
    const imageValue = imagesList.filter(eachValue => eachValue.thumbnailUrl === thumbnailUrl);
    const {imageUrl} = imageValue[0];
    if(imageUrl === imgUrl){
      const newImageUrl = imagesList[Math.floor(Math.random()*imagesList.length)].imageUrl
  ...
  render(){
    const{score,time,category,imgUrl,isTrue} = this.state
    const thumbnailList = imagesList.filter(eachValue => eachValue.category === category)
    return(
            ...
            <ul className='tab-elements'>
              {tabsList.map(tabDetails => (
                <TabItem 
                  ...
                  isActive = {tabDetails.tabId === category}  
                />
              ))}
            </ul>
